...
public class UserDBHandler extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        String query = "CREATE TABLE " + TABLE_NAME + " (" + ID_COL + " INTEGER PRIMARY KEY AUTOINCREMENT, " + NAME_COL + " VARCHAR(50) NOT NULL, " + EMAIL_COL + " VARCHAR(50) NOT NULL UNIQUE, " + PASSWORD_HASH_COL + " CHAR(64) NOT NULL, " + PASSWORD_SALT_COL + " CHAR(32) NOT NULL, " + ROLE_COL + " VARCHAR(9) NOT NULL, " + PHOTO_COL + " VARCHAR(50), " + ORGANIZATION_COL + " VARCHAR(100), " + CONTACT_COL + " VARCHAR(50), " + RESET_OTP_COL + " VARCHAR(6)," + RESET_TIME_COL + " BIGINT DEFAULT -1" + ")";

        db.execSQL(query);

        query = "CREATE TABLE " + TABLE1 + "(" + T1COL1 + " INTEGER PRIMARY KEY," + T1COL2 + " INTEGER," + T1COL3 + " TEXT," + T1COL4 + " TEXT," + T1COL5 + " TEXT," + T1COL6 + " TEXT," + T1COL7 + " TEXT," + T1COL8 + " INTEGER)";
        db.execSQL(query);

        query = "CREATE TABLE " + TABLE2 + "(" + T2COL1 + " INTEGER PRIMARY KEY," + T2COL2 + " INTEGER," + T2COL3 + " INTEGER," + T2COL4 + " TEXT," + T2COL5 + " TEXT," + // CHECK IF DATETIME WORKS
     ...
     *
     * @param user a User object containing the user details. Name, email,
     *             password and role are mandatory, while photo, organization and contact
     *             are optional (in which case they're null)
     * @return true if successful and false if fail
     * @throws IllegalArgumentException if the name, email, password, or role
     *                                  value of the User is null
     */
        ...
        if (cursor.getCount() > 0) {
            ...
            do {
                if (cursor.getString(6) == null) {
                    userHashedArray.add(new UserHashed(cursor.getString(1), cursor.getString(2), cursor.getString(3), cursor.getString(4), cursor.getString(5), null, cursor.getString(7), cursor.getString(8), cursor.getInt(0)));
                } else {
                    userHashedArray.add(new UserHashed(cursor.getString(1), cursor.getString(2), cursor.getString(3), cursor.getString(4), cursor.getString(5), cursor.getString(6), cursor.getString(7), cursor.getString(8), cursor.getInt(0)));
                }
    ...
    public UserHashed getUserFromEmail(String email) {
        ...
        if (cursor.getCount() > 0) {
            cursor.moveToFirst();
            if (cursor.getString(6) == null) {
                userHashed = new UserHashed(cursor.getString(1), cursor.getString(2), cursor.getString(3), cursor.getString(4), cursor.getString(5), null, cursor.getString(7), cursor.getString(8), cursor.getInt(0));
            } else {
                userHashed = new UserHashed(cursor.getString(1), cursor.getString(2), cursor.getString(3), cursor.getString(4), cursor.getString(5), cursor.getString(6), cursor.getString(7), cursor.getString(8), cursor.getInt(0));
            }
        }
        cursor.close();
        return userHashed;
     ...
     * @return true if a user with said email exists in the database, false if
     * not
     */
     ...
     * parameter. Any parameter that is not null will replace an existing value
     * with a new one. The email value is what's used to locate the row and thus
     * cannot be changed by this method. This method also cannot change the
     * password.
     *
    ...
    /**
     * Generates a new salt and hash based on the given password which then
     * replaces the existing salt and hash of the user with the given email.
     *
     * @param email    the email of the user whose password will be changed
     * @param password the new password
     * @return true if successful and false if unsuccessful or if a user with
     * the given email cannot be found in the database
     */
    ...
    /**
     * Returns true of the password matches the hash in the database retrieved
     * from the email value. Returns false otherwise
     *
    ...
    /**
     * Creates an OTP with a length of 6 characters, saves it along with the
    ...
    /**
     * Creates an OTP with a custom length, saves it along with the time of
     ...
     * @return the OTP code as a String, or null if the user has no OTP or
     * doesn't exist.
     */
     ...
     *
     * @param email the email of the user whose OTP creation time you want to
     *              receive.
     ...
     *
     * @param email the email of the user whose OTP code and time you want to
     *              delete.
    ...
    /**
     * Validates an OTP code against the OTP entry in the database. This method
     * has a time limit of 1 hour, meaning it will always return false if the
     * method is called against an otp generated more than 1 hour ago. If the
     * validation is successful then this method will also erase the OTP entry
     * in the database. Therefore this method can only return true once for each
     * OTP generated.
     *
     * @param email the email of the user whose OTP code you want to validate.
     * @param otp   the OTP code that will be compared to the database entry, case
     *              sensitive.
     * @return true if the given OTP matches the database entry and the time
     * difference doesn't exceed the given time limit, returns false in every
     * other case.
     */
    ...
    /**
     * Validates an OTP code against the OTP entry in the database. This method
     * can be given a custom time limit, meaning it will always return false if
     * the method is called against an otp generated more than
     * "timeLimitInHours" hours ago. If the validation is successful then this
     * method will also erase the OTP entry in the database. Therefore this
     ...
     *                         if the given time limit is 24 then this function will fail the validation
     *                         if 24 hours has passed since the OTP was generated.
     * @return true if the given OTP matches the database entry and the time
     * difference doesn't exceed the given time limit, returns false in every
     * other case.
     */
     ...
     *
     * @param orgID The unique identifier of the organizer whose events are to
     *              be retrieved.
     * @return An {@code ArrayList<Event>} containing all events associated with
     * the given organizer.
     */
    public ArrayList<Event> viewEventsForOrganizer(String orgID) {
        ...
        if (cursor.getCount() > 0) {
            cursor.moveToFirst();
            do {
                EventArray.add(new Event(cursor.getInt(0), cursor.getInt(1), cursor.getString(2), cursor.getString(3), cursor.getString(4), cursor.getString(5), cursor.getString(6), cursor.getInt(7)));
            } while (cursor.moveToNext());
        }
        cursor.close();
        return EventArray;
    ...
    public boolean updateEvent(int eveID, int orgID, String ti, String des, String sDate, String eDate, String loc, int cap) {
        ...
        values.put(T1COL7, loc);
        values.put(T1COL8, cap);
        int u = sqLiteDatabase.update(TABLE1, values, "event_id=?", new String[]{Integer.toString(eveID)});
        if (u > 0) {
            return true;
        } else {
            return false;
    ...
    public boolean delEvent(int eveID) {
        SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
        int d = sqLiteDatabase.delete(TABLE1, "event_id=?", new String[]{Integer.toString(eveID)});
        if (d > 0) {
            return true;
        } else {
            return false;
     ...
     *
     * @param atteID The unique identifier of the attendee whose invitations are
     *               to be retrieved.
    ...
    public ArrayList<Invitation> viewInvitationsForAttendeeID(String atteID) {
        ...
        Cursor cursor = sqLiteDatabase.rawQuery("SELECT * FROM " + TABLE2 + " WHERE " + T2COL3 + " = " + atteID, null);

        if (cursor.getCount() > 0) {
            cursor.moveToFirst();
            do {
                InvitationArray.add(new Invitation(cursor.getInt(0), cursor.getInt(1), cursor.getInt(2), cursor.getString(3), cursor.getString(4), cursor.getString(5)));
            } while (cursor.moveToNext());
        }
        cursor.close();
     ...
     * @return {@code true} if the invitation was successfully updated,
    ...
    public boolean updateInvitation(int invID, int eveID, int atteID, String status, String creaTime, String respTime) {
        ...
        // values.put(T2COL6,respTime); //Create a method to return this value to the
        // table
        int u = sqLiteDatabase.update(TABLE2, values, "invitation_id=?", new String[]{Integer.toString(invID)});
        if (u > 0) {
            return true;
        } else {
            return false;
     ...
     * @return {@code true} if the invitation was successfully deleted,
     * otherwise {@code false}.
     */
    public boolean delInvitation(int invID) {
        SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
        int d = sqLiteDatabase.delete(TABLE2, "invitation_id=?", new String[]{Integer.toString(invID)});
        if (d > 0) {
            return true;
        } else {
            return false;
    ...
    /**
     * Retrieves an event from the database based on its id. if the event does
    ...
    public Event getEventById(int eventId) {
        ...
        Cursor cursor = sqLiteDatabase.rawQuery("SELECT * FROM " + TABLE1 + " WHERE " + T1COL1 + " = " + eventId, null);
        Event event = null;
        if (cursor.getCount() > 0) {
            Log.d("UserDBHandler", "Event found with id: " + eventId + " cursor count: " + cursor.getCount() + " cursor position: " + cursor.getPosition() + " cursor row count: " + cursor.getCount() + " cursor column count: " + cursor.getColumnCount() + " cursor");
            cursor.moveToFirst();
            event = new Event(cursor.getInt(0), cursor.getInt(1), cursor.getString(2), cursor.getString(3), cursor.getString(4), cursor.getString(5), cursor.getString(6), cursor.getInt(7));
        } else {
            Log.d("UserDBHandler", "Event not found with id: " + eventId);
        }
        cursor.close();
        return event;
    }
    ...
    /**
     * Retrieves a list of events for a specific attendee.
     * This method queries the database to find events where the attendee
     * is listed in the invitations table, regardless of the invitation status.
     * The events are returned as an ArrayList of {@link Event} objects.
     *
     * @param attendeeId the attendee id
     * @return an ArrayList of events for the given attendee
     */
    public ArrayList<Event> getAllEventsForAttendee(int attendeeId) {
        SQLiteDatabase sqLiteDatabase = this.getReadableDatabase();
        ArrayList<Event> events = new ArrayList<>();
        Cursor cursor = sqLiteDatabase.rawQuery("SELECT * FROM " + TABLE1 + " WHERE " + T1COL1 + " IN (SELECT " + T2COL2 + " FROM " + TABLE2 + " WHERE " + T2COL3 + " = " + attendeeId + ")", null);
        if (cursor.getCount() > 0) {
            cursor.moveToFirst();
            do {
                events.add(new Event(cursor.getInt(0), cursor.getInt(1), cursor.getString(2), cursor.getString(3), cursor.getString(4), cursor.getString(5), cursor.getString(6), cursor.getInt(7)));
            } while (cursor.moveToNext());
        }
        cursor.close();
        return events;
    ...
    /**
     * Retrieves a list of accepted events for a specific attendee
     *
     * @param attendeeId the attendee id
     * @return an ArrayList of accepted events for the given attendee
     */
    public ArrayList<Event> getAcceptedEventsForAttendee(int attendeeId) {
        SQLiteDatabase sqLiteDatabase = this.getReadableDatabase();
        ArrayList<Event> EventArray = new ArrayList<>();
        Cursor cursor = sqLiteDatabase.rawQuery("SELECT * FROM " + TABLE1 + " WHERE " + T1COL1 + " IN (SELECT " + T2COL2 + " FROM " + TABLE2 + " WHERE " + T2COL3 + " = " + attendeeId + " AND " + T2COL4 + " = 'ACCEPTED')", null);
        if (cursor.getCount() > 0) {
            cursor.moveToFirst();
            do {
                EventArray.add(new Event(cursor.getInt(0), cursor.getInt(1), cursor.getString(2), cursor.getString(3), cursor.getString(4), cursor.getString(5), cursor.getString(6), cursor.getInt(7)));
    ...
    /**
     * Retrieves a list of past events for a specific attendee that have already occurred
     *
     * @param attendeeId the attendee id
     * @return an ArrayList of past events for the given attendee
     */
    public ArrayList<Event> getPastEventsForAttendee(int attendeeId) {
        SQLiteDatabase sqLiteDatabase = this.getReadableDatabase();
        ArrayList<Event> EventArray = new ArrayList<>();
        ...
        String query = "SELECT * FROM " + TABLE1 + " WHERE " + T1COL1 + " IN (" + "    SELECT " + T2COL2 + "    FROM " + TABLE2 + "    WHERE " + T2COL3 + " = ? AND " + T2COL4 + " = ?" + ") AND " + T1COL6 + " < ?";

        // Execute the query with parameters
        Cursor cursor = sqLiteDatabase.rawQuery(query, new String[]{String.valueOf(attendeeId), "ACCEPTED", currentDateTime});
    ...
    /**
     * Retrieves a list of attendees for a specific event that have accepted the invitation.
     *
     * @param eventId the event id
     * @return an ArrayList of attendees for the given event
     */
    public ArrayList<UserHashed> getAttendeesForEventId(int eventId) {
        SQLiteDatabase sqLiteDatabase = this.getReadableDatabase();
        ArrayList<UserHashed> attendeeArray = new ArrayList<>();
        Cursor cursor = sqLiteDatabase.rawQuery("SELECT * FROM " + TABLE_NAME + " WHERE " + ID_COL + " IN (SELECT " + T2COL3 + " FROM " + TABLE2 + " WHERE " + T2COL2 + " = " + eventId + " AND " + T2COL4 + " = 'ACCEPTED')", null);

        if (cursor.getCount() > 0) {
            cursor.moveToFirst();
            do {
                if (cursor.getString(6) == null) {
                    attendeeArray.add(new UserHashed(cursor.getString(1), cursor.getString(2), cursor.getString(3), cursor.getString(4), cursor.getString(5), null, cursor.getString(7), cursor.getString(8), cursor.getInt(0)));
                } else {
                    attendeeArray.add(new UserHashed(cursor.getString(1), cursor.getString(2), cursor.getString(3), cursor.getString(4), cursor.getString(5), cursor.getString(6), cursor.getString(7), cursor.getString(8), cursor.getInt(0)));
                }
            } while (cursor.moveToNext());
        }
        cursor.close();
